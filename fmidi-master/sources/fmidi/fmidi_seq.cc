...
// ROBOX RECOMP MODIFICATION: the game's music engine always plays songs at a
// fixed 120 BPM regardless of the tempo meta events in the file. Non-zero =
// lock the sequencer to this tempo (microseconds per quarter note) and ignore
// every "set tempo" (meta 0x51) event. 500000 us = 120 BPM.
// Default LOCKED at 500000 us/quarter = 120 BPM: the tempo must already
// be fixed when the very first song is parsed, otherwise a song whose
// first tempo event differs starts at its own speed and only snaps to
// 120 later. Set to 0 to honour the files' tempo events again.
extern "C" uint32_t fmidi_fixed_tempo = 500000;
...
void fmidi_seq_rewind(fmidi_seq_t *seq)
{
    ...
    for (unsigned i = 0; i < ntracks; ++i) {
        ...
        timing->tempo = fmidi_fixed_tempo ? fmidi_fixed_tempo : 500000;
    ...
    for (unsigned i = 0; i < ntracks; ++i) {
        ...
        while ((evt = fmidi_smf_track_next(smf, &it)) &&
            ...
            if (id == 0x51 && evt->datalen == 4 && !fmidi_fixed_tempo) {  // set tempo
...
bool fmidi_seq_next_event(fmidi_seq_t *seq, fmidi_seq_event_t *sqevt)
{
    ...
    if (evt->type == fmidi_event_meta) {
        if (evt->data[0] == 0x51 && evt->datalen == 4 && !fmidi_fixed_tempo) {  // set tempo
            const uint8_t *d24 = &evt->data[1];
