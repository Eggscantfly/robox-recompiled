        ...
        if (mNextNativeState == NativeState.RESUMED) {
            if (mSurface.mIsSurfaceReady && mHasFocus && mIsResumedCalled) {
                if (mSDLThread == null) {
                    ...
                    /* ROBOX: give SDL_main a huge stack.
                     *
                     * This thread runs the recompiled PowerPC guest, whose deep
                     * call chains become deep host call chains. The JVM default
                     * (~1 MB) overflows during level loading -- the same crash
                     * the web build hit with a 64 MB stack, and the reason the
                     * Windows build reserves 512 MB via -Wl,--stack. The
                     * runtime's own runaway-recursion guard only trips at
                     * 300 MB, so the guest legitimately uses hundreds of MB.
                     *
                     * ROBOX_GUEST_STACK_BYTES covers guest WORKER threads
                     * (sdk/os.c), but never applied here because this thread is
                     * created on the Java side. */
                    mSDLThread = new Thread(null, new SDLMain(), "SDLThread",
                                            512L * 1024L * 1024L);
